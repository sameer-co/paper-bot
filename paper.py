...
async def fetch_indicators():
    try:
        ...
        if rsi is None or rsi_ema is None: return None, None, None, None
...
async def monitor_trade(price, bot):
    ...
    # STAGE 1: Hit 1.5R -> Trail SL to +0.8R (Guaranteed Profit)
...
async def close_trade(exit_price, reason, bot):
    ...
    # Accurate Stat Sorting
    if "FINAL TARGET" in reason:
        stats['wins_final_target'] += 1
    elif "TRAILED SL HIT" in reason:
        # If PnL is positive, it's a win. If negative (Stage 0), it's a loss.
        if pnl > 0:
    ...
    total_wins = stats['wins_final_target'] + stats['wins_trailed']
    win_rate = (total_wins / stats['total_trades']) * 100
...
async def main():
    ...
    # Modern async initialization for Telegram Bot
                    ...
                    while True:
                        ...
                        if 'k' in data:
                            ...
                            if data['k']['x']: # Candle Close
                                ...
                                if rsi is not None and not active_trade:
                                    # RSI crossover EMA Strategy
                                    if prsi <= pema and rsi > rsi_ema:
                                        # Fetch SL reference
                                        api_url = f"https://api.binance.com/api/v3/klines?symbol={SYMBOL}&interval=5m&limit=1"
                                        resp = await http_client.get(api_url)
                                        candle = resp.json()
                                        low_price = float(candle[0][3]) * 0.9995 
                                        
                                        risk_amount = stats['balance'] * stats['risk_percent']
                                        
                                        active_trade = {
                                            'entry': price, 'initial_sl': low_price, 'sl': low_price,
                                            'risk_usd': risk_amount, 'stage0_hit': False, 
                                            'stage1_hit': False, 'stage2_hit': False
                                        }
